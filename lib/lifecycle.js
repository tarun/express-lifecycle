...
function endResponse(lifecycle, req, res) {
    ...
    res.end = function lifecycleEnd(chunk, encoding) {
        ...
        if (res.lifecycle_done) {
            debug('actual end');
            _end.call(_res, _chunk || chunk, _encoding || encoding);
        } else {
            // save params for later use.
            _chunk = chunk;
            _encoding = encoding;

            lifecycle.after.handle(_req, _res, function() {
                debug('after -> next')
                lifecycle._close.handle(_req, _res, function() {
                    debug('close -> next')
                });
            });
        }
...
function Lifecycle() {
    ...
    this._close.use(function(req, res, next) {
        ...
        if (res.lifecycle_end_triggered === true) {
            debug('lifecycle_end_triggered: true')
            res.end();
        } else {
            next();
        }
